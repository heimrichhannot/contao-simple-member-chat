...
| Low-priority asynchronous delivery | **Corrected 2026-09-18.** `SendChatPushMessage` now carries `Symfony\Component\Messenger\Attribute\AsMessage` with `contao_prio_low`, matching the core's own messages (`vendor/contao/core-bundle/src/Messenger/Message/{CrawlMessage,SearchIndexMessage}.php`). `vendor/contao/core-bundle/src/Messenger/Message/LowPriorityMessageInterface.php` triggers a deprecation for Contao 5.6 and stops working in Contao 6, so the marker interface was removed. The host `vendor/contao/manager-bundle/skeleton/config/config.yaml` routes the transport to Doctrine and configures web/cron workers. |
