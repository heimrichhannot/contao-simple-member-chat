...
## Phase 4b polling resilience (manual acceptance pending)

Both rows are **not verified in a browser**. The deterministic DDEV client
harness reproduces both failures against the original script and passes with
the fix. Use rebuilt assets and a fresh full page load before each check.

| Defect | Exact reproduction | Expected result |
| --- | --- | --- |
| Polling freezes after cache teardown | Open the logged-in Alice/Bob chat and record message GETs in Network for 9 seconds. In DevTools execute `document.dispatchEvent(new Event('turbo:before-cache'))`; wait 10 seconds without navigating: no chat polls. Execute `window.dispatchEvent(new PageTransitionEvent('pageshow', {persisted: true}))`, without dispatching `turbo:load`; observe for 10 seconds. Repeat with `persisted: false`, and with visible `visibilitychange` or `turbo:render` as the only recovery signal. Finally dispatch `turbo:render` and `turbo:load` together and observe another 10 seconds. Also navigate away/back using native browser history to exercise actual bfcache if the browser permits it. | Polling stops during teardown, resumes on the recovery signal within the configured interval (messages: 4 seconds), and repeated signals do not double the request rate. Native pagehide stops polling too. Synthetic pageshow proves the event path only; actual bfcache restoration must be recorded separately. |
| Long intervals starve on visibility changes | On an existing badge-enabled page with `data-chat-poll-interval="30000"`, reload and start timing after frame discovery. Switch to another tab at 4 seconds and return at 5 seconds; repeat at 9/10, 14/15, 19/20 and 24/25 seconds, then remain visible through 38 seconds. Observe badge reloads, excluding its initial load. After a poll, hide at +5 seconds and return at +40 seconds. Repeat while offline, then restore the network. | First badge poll occurs around 30 seconds after discovery (timer throttling may add delay), rather than 30 seconds after the last return. No requests while hidden. Returning after an elapsed deadline polls immediately; failures retain capped backoff and success restores the normal interval. Hidden mobile list frames remain paused. |

## Phase 6 backend fixes (UI acceptance not verified)

The booted-host smoke renders both registered label callbacks and the core
header formatter in English/German. No backend login was performed in this
phase; complete these steps as an authorized backend user after refreshing the
host cache. Record UI results separately from automated evidence.

| Defect | Exact action | Expected result |
| --- | --- | --- |
| Conversation list HTTP 500 | Open `/contao?do=member_chat` with existing conversations, first with English and then German backend language. Inspect Network and visible labels. Include a missing/deleted member and a display name containing markup or a percent sign when suitable demo fixtures exist. | HTTP 200, no ValueError; member pair and last-message time appear with translated wording. Markup remains escaped text and literal percent signs survive. An empty conversation shows the existing em dash. |
| Raw child-list timestamp | Choose a conversation whose database `lastMessageAt` is positive; note its value and the host `datimFormat`/timezone. Open `/contao?do=member_chat&table=tl_chat_message&id=<conversation-id>` via its Messages operation. Repeat in English/German. For the smoke fixture timestamp `1789653654`, the current host formatter returned `2026-09-17 16:00`. | Header label is Last message / Letzte Nachricht; value follows the host date/time format and timezone, never the raw Unix integer. Message author/time/excerpt still render escaped. For `lastMessageAt = 0`, the header field is omitted, with no Unix-epoch date. |
